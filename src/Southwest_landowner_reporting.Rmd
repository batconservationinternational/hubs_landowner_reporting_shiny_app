...
### <span style="color: darkblue;"> __`r (dat_count %>% dplyr::filter(year == this_year, pres_abs == T) %>% group_by(year) %>% summarise(n = n_distinct(species)) %>% dplyr::pull(n))` bat species were detected in 2021__ across all survey locations in this report. Read on to learn more about which species were found where. </span>
